...
def build_features(cfg: DictConfig) -> None:
    ...
    if not metadata_path.exists():
        raise FileNotFoundError(f"❌ metadata.csv not found at {metadata_path}. Run ingest + validate first.")

    print("🔄 Loading metadata...")
    df = pd.read_csv(metadata_path)
    
    # === 1. Stratified Split ===
    print(f"📊 Performing stratified split (70/15/15) with seed={cfg.split.random_seed}...")
    train_df, temp_df = train_test_split(
        df, test_size=(1 - cfg.split.train_ratio),
        stratify=df["label"], random_state=cfg.split.random_seed
    )
    val_ratio_adj = cfg.split.val_ratio / (cfg.split.val_ratio + cfg.split.test_ratio)
    val_df, test_df = train_test_split(
        temp_df, test_size=(1 - val_ratio_adj),
        stratify=temp_df["label"], random_state=cfg.split.random_seed
    )
    ...
    print("✅ Feature artifacts saved (train.csv, val.csv, test.csv, features_config.json)")
